...
export class DiscountFormComponent {
  ...
  constructor(private formBuilder: FormBuilder,private soldService:SoldServices,private discountService:DiscountService, private router:Router, private route:ActivatedRoute,private comboService:ComboService){}
 ...
 combos:Combo[]
  ...
  initializeForm(): void {
    ...
    this.discountForm.get('discountType').valueChanges.subscribe(value => {
      if (value === 'FLAT') {
        // If discountType is 'Flat', disable buyX and getY inputs
        this.discountForm.get('buyX').setValue('');
      this.discountForm.get('getY').setValue('');
        this.discountForm.get('buyX').disable();
        this.discountForm.get('getY').disable();
      } else {
        // Otherwise, enable buyX and getY inputs
        this.discountForm.get('buyX').enable();
        this.discountForm.get('getY').enable();
      }
    });
  }
