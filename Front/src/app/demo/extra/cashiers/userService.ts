...
export class UserService {
  private apiUrl = 'http://localhost:8888/admin/realms/PFE';
  ...
  getUserAttribute(userId: string, attribute: string): Observable<any> {
    this.token = localStorage.getItem('token');
    ...
    const url = `${this.apiUrl}${userId}/attributes/${attribute}`;
    return this.http.get<any[]>(url, this.httpOptions);
  }
