...
loadRoles(): void {
  this.userService.getRealmRoles().subscribe(
    roles => {
      this.roles = roles.filter(role => !["Developper","ADMIN","offline_access", "default-roles-pfe", "uma_authorization"].includes(role.name));
...
assigncashierId(userId:string, cashierId:string){
  this.userService.updateCashierId(userId.toString(), cashierId.toString()).subscribe(
    () => {
      console.log('cashierId updated successfully');
      this.load();
    
    },
    error => {
      console.error('Error updating cashierId:', error);
    }
  );
}
