...
export class StockServices {
    private stockProductUrl = ' http://localhost:8085/gateway/STOCK/stock-products';
    ...
    token = localStorage.getItem("token")
      ...
      checkStockAvailability(stockEquivalents: any[], quantity: number): Observable<boolean> {
        const url = `${this.stockProductUrl}/check-stock-availability?quantity=${quantity}`;
        return this.http.post<boolean>(url, stockEquivalents, this.httpOptions);
      }
