...
export class AddestabComponent {
    ...
    this.establishmentService.getEstablishmentById(id).subscribe(
      establishment => {
        ...
        this.establishmentForm.patchValue({
          ...
          cashOutRatio: establishment.cashOutRatio,
          ScannerCheck:establishment.scanSystem
  ...
  addEstablishment(): void {
    ...
    this.estab.cashOutRatio = this.establishmentForm.get('cashOutRatio').value;
    this.estab.scanSystem = this.establishmentForm.get('ScannerCheck').value;

    if (this.estab.fidelitySystem === false) { this.estab.fidelityRatio = 0; this.estab.cashOutRatio = 0;}
  ...
  checkFidelityRatio(): boolean {
    ...
    const cashOutRatio = this.establishmentForm.get('cashOutRatio').value;


    if (fidelityCheck && fidelityRatio > 0 && cashOutRatio>0) {
      ...
      this.notificationService.showError("", "Les Ratio de fidélité doivent être > 0");
