...
export class EstablishmentService {
  private apiUrl = 'http://localhost:8085/gateway/STOCK/establishments';
  ...
  httpOptions = {
    ...
    updateTableStatus(tableId: number, status: boolean): Observable<Tables> {
      return this.http.put<Tables>(`${this.apiUrl}/tables/${tableId}/status?status=${status}`, {}, this.httpOptions);
    }
 ...
 addEstablishment(establishment: Establishment): Observable<Object> {
  ...
  getTableById(id: number): Observable<Tables> {
    return this.http.get<Tables>(`${this.apiUrl}/table/${id}`, this.httpOptions);
  }
